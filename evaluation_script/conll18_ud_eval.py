...
def load_conllu(file):
    ...
    while True:
        ...
        if not line:
            ...
            def process_word(word, cycles, head_outside):
                if word.parent == "remapping":
                    cycles += 1
                    # print("There is a cycle in a sentence")
                    # raise UDError("There is a cycle in a sentence")
                if word.parent is None:
                    head = int(word.columns[HEAD])
                    if head < 0 or head > len(ud.words) - sentence_start:
                        head_outside += 1
                        # print("HEAD '{}' points outside of the sentence")
                        # raise UDError("HEAD '{}' points outside of the sentence".format(_encode(word.columns[HEAD])))
                    elif head:
                        parent = ud.words[sentence_start + head - 1]
                        word.parent = "remapping"
                        cycles, head_outside = process_word(parent, cycles, head_outside)
                        word.parent = parent
                return cycles, head_outside

            for word in ud.words[sentence_start:]:
                cycles, head_outside = process_word(word, cycles, head_outside)
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) < 1:
                no_root += 1
                # print("There are no roots in a sentence")
                # for word in ud.words[sentence_start:]:
                #     print(word.columns)
    ...
    print("There were '{}' cycles.".format(cycles))
    print("There were '{}' heads outside of sentence.".format(head_outside))
    print("There were '{}' sentences with multiple roots.".format(multiple_root))
    print("There were '{}' sentences without root.".format(no_root))
